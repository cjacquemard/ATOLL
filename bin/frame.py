...
class Selector:
	protein = 'protein or resname '+' '.join([aa['tl_code'] for aa in config._RESLIB['amino_acids']]) # MODIFY !!!
	...
	def split(cls, selection):
		...
		for index_range in selection.split('+'):
			...
			if len(sub_indices) == 1:
				indices.append(int(sub_indices[0]))
...
class Basic:
	...
	def renumber_protein_residues(self):
		for residue in self.protein.residues:
			structure_resid = residue.resid
			structure_position = self.numbering.structure.get_position_by_resid(structure_resid)
			reference_position = self.numbering.get_reference_position_by_structure_position(structure_position)
			reference_resid = self.numbering.reference.get_resid(reference_position)

			residue.resid = reference_resid
	...
	def check_path(self):
		...
		for filename in filenames:
			...
			if fileext in self._SUPPORTED_TOPOLOGY_FILEEXTS:
				if not os.access(filepath, os.R_OK):
					logger.warning(f'Topology file {filepath} is not readable')
				else:
					topology_filepathes.append(filepath)

			if fileext in self._SUPPORTED_COORDINATE_FILEEXTS:
				if not os.access(filepath, os.R_OK):
					logger.warning(f'Coordinate file {filepath} is not readable')
				else:
		...
		if n_top == 0:
			...
			if any(check_fileexts):
				logger.critical(f'Found mixed file type (single structure files and coordinate files) in {self.dirpath}')
			else:
				logger.critical(f'No topology file was found in {self.dirpath} directory')
		elif n_top > 1:
			logger.critical(f'{n_top} topology files were found in {self.dirpath} directory. Only 1 is allowed.')
		else:
			self.topology_filepath = topology_filepathes[0]

		if not self.coordinate_filepathes:
			logger.critical(f'No coordinate file was found in {self.dirpath} directory')
...
class Output(path.DirChecker):
	...
	def check_path(self):
		if not os.path.exists(self.root_dirpath):
			logger.critical(f'{self.root_dirpath} path is not found')

		if not os.path.isdir(self.root_dirpath):
			logger.critical(f'{self.root_dirpath} path is not a directory')

		if not os.access(self.root_dirpath, os.X_OK):
			logger.critical(f'{self.root_dirpath} is not executable')

		if not os.access(self.root_dirpath, os.W_OK):
			logger.critical(f'{self.root_dirpath} is not writable')

		if not os.access(self.root_dirpath, os.R_OK):
			logger.critical(f'{self.root_dirpath} is not readable')

		if os.path.exists(self.dirpath) and not self.overwrite:
			logger.critical(f'{self.dirpath} already exists')
		elif os.path.exists(self.dirpath) and self.overwrite:
			logger.info(f'{self.dirpath} already exists')
